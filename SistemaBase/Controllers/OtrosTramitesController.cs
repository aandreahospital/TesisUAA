...
{
    public class OtrosTramitesController : Controller
    {
        private readonly DbvinDbContext _dbContext;

        public OtrosTramitesController(DbvinDbContext context)
        {
            _dbContext = context;
        }

        //FILTRO QUE AUTORIZA AL USUARIO PARA ACCEDER AL CONTROLADOR CON EL PERMISO
        [TypeFilter(typeof(AutorizarUsuarioFilter), Arguments = new object[] { "RMOTRTRA", "Index", "OtrosTramites" })]
        ...
        public IActionResult BuscarImagen(string nroBoleta)
        {
            ...
            if (nroBoleta != null)
            {
                var transaccion = _dbContext.RmTransacciones.Where(t => t.NroBoleta == nroBoleta).FirstOrDefault(t => t.TipoOperacion == "6");
                var nroTrans = transaccion?.NumeroEntrada;
                var result = _dbContext.RmMarcasSenales.FirstOrDefault(p => p.NumeroEntrada.Equals(nroTrans));
                ...
                if (result != null)
                {
                    try
                    {
                        ...
                        var estableDis = _dbContext.RmMarcasXEstabs.FirstOrDefault(m => m.IdTransaccion == transaccion.IdTransaccion);
                        var distrito = _dbContext.RmDistritos.FirstOrDefault(d => d.CodigoDistrito.ToString() == estableDis.CodDistrito);
        ...
        public async Task<IActionResult> GetTitular(string codPersona)
        {
            try
            {
                var mesaEntrada = await _dbContext.RmMesaEntrada.Where(m => m.NroBoleta != null).FirstOrDefaultAsync(p => p.NroDocumentoTitular == codPersona);
        ...
        public async Task<JsonResult> CargarOperacionesBoleta(string nroBoleta)
        {
            ...
            var transaccion = _dbContext.RmTransacciones.OrderBy(t => t.FechaAlta).FirstOrDefault(t => t.NroBoleta == nroBoleta);
            var boletaTrans = transaccion?.NroBoleta;
            //Para obtener transaccion 
            var queryTitularEntrada = queryTransaccion.Where(t => t.NroBoleta == boletaTrans).AsQueryable().Join(queryEntrada,
        ...
        public async Task<IActionResult> CambiarEstado(decimal id, string nuevoEstado, string comentario, string tipoOperacion)
        {
            try
            {
                ...
                {
                    var enrevision = await _dbContext.RmEstadosEntrada.FirstOrDefaultAsync(e => e.DescripEstado == "En Revisión");
                    var transaccion = await _dbContext.RmTransacciones.FirstOrDefaultAsync(p => p.NumeroEntrada == id && p.EstadoTransaccion == enrevision.CodigoEstado.ToString());
                    if (transaccion != null)
                    {
        ...
        [HttpPost] //FILTRO QUE AUTORIZA AL USUARIO PARA ACCEDER AL CONTROLADOR CON EL PERMISO
        [TypeFilter(typeof(AutorizarUsuarioFilter), Arguments = new object[] { "RMOTRTRA", "Create", "OtrosTramites" })]
        ...
        public async Task<IActionResult> Create(InformeCustom transaccion)
        {
            ...
            var rmTransaccion = await _dbContext.RmTransacciones.FirstOrDefaultAsync(t => t.NumeroEntrada == transaccion.NumeroEntrada && t.EstadoTransaccion == rmMesaEntrada.EstadoEntrada.ToString());
            ...
            if (rmTransaccion != null)
            {
                ...
                if (rmMesaEntrada != null)
                {
                    ...
                    rmTransaccion.NroBoleta = transaccion.NroBoleta;
