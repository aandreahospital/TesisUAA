...
namespace SistemaBase.Controllers
{
    ...
    public class InformeEntradaDiseñoController : Controller
    {
        private readonly DbvinDbContext _dbContext;
        public InformeEntradaDiseñoController(DbvinDbContext context)
        {
            _dbContext = context;
        }
        //FILTRO QUE AUTORIZA AL USUARIO PARA ACCEDER AL CONTROLADOR CON EL PERMISO
         [TypeFilter(typeof(AutorizarUsuarioFilter), Arguments = new object[] { "RMIEPD", "Index", "InformeEntradaDiseño" })]
        ...
        {
            var tipoSoli = _dbContext.RmTipoSolicituds.FirstOrDefault(t => t.DescripSolicitud == "INSCRIPCION");
            var estadoRecibido = _dbContext.RmEstadosEntrada.FirstOrDefault(e => e.DescripEstado == "Enviado a Diseño");
            var tipoSolicitud = _dbContext.RmTipoSolicituds.AsQueryable();
            var mesaEntrada = _dbContext.RmMesaEntrada.Where(m => m.TipoSolicitud == 1).AsQueryable();
    ...
    .AsQueryable();
            ...
            {
                mesaEntrada = mesaEntrada.Where(m => m.CodigoOficina == parametros.CodigoOficina).AsQueryable();
            }

            // Esta línea une las tablas y en función de condiciones específicas,
            // creando un nuevo objeto de tipo EntradasCargas. El método se aplica para eliminar entradas duplicadas
            ...
            var resultados = queryFinal.OrderBy(e => e.FechaEntrada).ToList();
