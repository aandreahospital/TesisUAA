...
{
    public class OperacionSupervisor3CustomController : Controller
    {
        private readonly DbvinDbContext _dbContext;
        public OperacionSupervisor3CustomController(DbvinDbContext context)
        {
            _dbContext = context;
        }
        //FILTRO QUE AUTORIZA AL USUARIO PARA ACCEDER AL CONTROLADOR CON EL PERMISO
        [TypeFilter(typeof(AutorizarUsuarioFilter), Arguments = new object[] { "RMOPJSUP", "Index", "OperacionSupervisor3Custom" })]
        public async Task<IActionResult> Index()
        {
            var queryFinal = (from me in _dbContext.RmMesaEntrada
                              ...
                              where (e.DescripEstado == "Aprobado/Supervisor" || e.DescripEstado == "Observado/Supervisor" || e.DescripEstado == "Nota Negativa/Supervisor")
        ...
        public async Task<IActionResult> AddCertificado(decimal nEntrada)
        {
            try
            {
                ...
                if (mesaEntrada != null)
                {
                    var transaccion = await _dbContext.RmTransacciones.OrderBy(o => o.FechaAlta).FirstOrDefaultAsync(p => p.NroBoleta == mesaEntrada.NroBoleta);
        ...
        {
            var rmMedidaPrenda = await _dbContext.RmMedidasPrendas.SingleOrDefaultAsync(p => p.NroEntrada == nEntrada);

            //ViewBag.Acreedor = new SelectList(_dbContext.Personas, "CodPersona", "Nombre", rmMedidaPrenda?.Acreedor?? "");
            //ViewBag.Deudor = new SelectList(_dbContext.Personas, "CodPersona", "Nombre", rmMedidaPrenda?.Deudor?? "");

            ViewBag.CodigoDistrito = new SelectList(_dbContext.RmDistritos, "CodigoDistrito", "DescripDistrito", rmMedidaPrenda?.CodDistrito ?? "");


            PrendaCustom prenda = new()
            {
                IdMedida = rmMedidaPrenda?.IdMedida ?? 0,
                NroEntrada = rmMedidaPrenda?.NroEntrada ?? 0,
                Libro = rmMedidaPrenda?.Libro ?? "",
                Folio = rmMedidaPrenda?.Folio ?? "",
                FechaInscripcion = rmMedidaPrenda?.FechaInscripcion ?? DateTime.MinValue,
                Acreedor = rmMedidaPrenda?.Acreedor ?? "",
                Deudor = rmMedidaPrenda?.Deudor ?? "",
                FechaVencimiento = rmMedidaPrenda?.FechaVencimiento ?? DateTime.MinValue,
                //NroBoleta = rmMedidaPrenda?.NroBoleta ?? "",
                //NroBoletaSenal = rmMedidaPrenda?.NroBoletaSenal ?? "",
                MontoPrenda = rmMedidaPrenda?.MontoPrenda ?? 0,
                CodDistrito = rmMedidaPrenda?.CodDistrito ?? "",
                Instrumento = rmMedidaPrenda?.Instrumento ?? ""
            };
        ...
        {
            var estadoEntrada = _dbContext.RmEstadosEntrada.FirstOrDefault(t => t.DescripEstado == "Aprobado/Firma");
            var mesaEntrada = _dbContext.RmMesaEntrada.FirstOrDefault(t => t.NumeroEntrada == nEntrada);
            ...
            if (mesaEntrada!=null)
            {
                ...
                RmMovimientosDoc movimientos = new()
                {
                    ...
                    FechaOperacion = DateTime.Now,
                    CodOperacion = "09", //Parametro para cambio de estado 
                    NroMovimientoRef = nEntrada.ToString(),
                    EstadoEntrada = mesaEntrada.EstadoEntrada
                };
                _dbContext.Add(movimientos);
                _dbContext.SaveChanges();

                var tipoSolicitud = _dbContext.RmTipoSolicituds.FirstOrDefault(t=>t.TipoSolicitud == mesaEntrada.TipoSolicitud);
                if (tipoSolicitud.DescripSolicitud == "INSCRIPCION" || tipoSolicitud.DescripSolicitud == "REINSCRIPCION" || tipoSolicitud.DescripSolicitud == "DUPLICADO" || tipoSolicitud.DescripSolicitud == "TRANSFERENCIA" || tipoSolicitud.DescripSolicitud == "ADJUDICACION" || tipoSolicitud.DescripSolicitud == "CAMBIO DE DENOMINACION" || tipoSolicitud.DescripSolicitud == "DONACION")
                {
