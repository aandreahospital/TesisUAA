...
{
    public class AprobadosParaSupervisionController : Controller
    {
        private readonly DbvinDbContext _dbContext;
        public AprobadosParaSupervisionController(DbvinDbContext context)
        {
            _dbContext = context;
        }
        //FILTRO QUE AUTORIZA AL USUARIO PARA ACCEDER AL CONTROLADOR CON EL PERMISO
        //[TypeFilter(typeof(AutorizarUsuarioFilter), Arguments = new object[] { "RMIEAPS", "Index", "AprobadosParaSupervision" })]
        ...
        private Reportes GetReporteData(Reportes parametros)
        {
            ...
            var estadoRecibido = _dbContext.RmEstadosEntrada.FirstOrDefault(e => e.DescripEstado == "Aprobado/JefeRegistral");
            
            //Tipos Operaciones que emitan titulos
            var tiposOperacion = new List<decimal> { 5, 6, 15, 2, 19, 7, 8, 3, 25 };
            ...
            var MOVI = queryMovimiento.ToList();
            var query = queryMovimiento
            .Where(e => e.EstadoEntrada == estadoRecibido.CodigoEstado)
            .AsQueryable()
            .Join(mesaEntrada,
                en => en.NroEntrada,
            ...
            var queryTipo = queryTransa.AsQueryable().Join(tipoSolicitud,
               ...
               (en, tipo) => new ListadoJefes { NumeroEntrada = en.NumeroEntrada, TipoSolicitud = tipo.DescripTipoOperacion, FechaEntrada = en.FechaEntrada, FechaAlta = en.FechaAlta, NomTitular = en.NomTitular, 
            ...
            var queryUsu = queryTipo.AsQueryable().Join(_dbContext.Usuarios,
              ...
              (en, tipo) => new ListadoJefes { NumeroEntrada = en.NumeroEntrada, TipoSolicitud = en.TipoSolicitud, FechaEntrada = en.FechaEntrada, FechaAlta = en.FechaAlta, NomTitular = en.NomTitular, 
          ...
          (en, tipo) => new ListadoJefes { NumeroEntrada = en.NumeroEntrada, TipoSolicitud = en.TipoSolicitud, FechaEntrada = en.FechaEntrada, FechaAlta = en.FechaAlta, NomTitular = tipo.DescripOficina ,
              NomOperador = en.NomOperador });
